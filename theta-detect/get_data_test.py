...
class GetData():
    ...
    def get_data(self):
        (x1, y1, x2, y2), handle = self.get_window_pos('这不是TNT - MuMu模拟器')
        try:
            # 发送还原最小化窗口的信息
        ...
        while x1 + x2 + y1 + y2 < 0:  # 有时候截取的尺寸不对，全是负值，故而做了一个坐标的判断
            (x1, y1, x2, y2), handle = get_data.get_window_pos('这不是TNT - MuMu模拟器')

        img = ImageGrab.grab((x1, y1 + 37, x2, y2))  # 截取tnt屏幕的照片   需要转化为np数组  37指的是上边框的厚度
        ...
        cv.imshow("img1", img[608:631, 130:185])  # 截取到的角度长55 高23
        theta = self.get_theta(img[608:631, 130:185])
        # print(theta)
    def get_wind(self):
        pass
    ...
    def get_theta(self):
        # 10model_test 是负号
        pic_theta = cv.imread("../../model_theta/_5_gray.png")  #   游戏中获取到的图片
